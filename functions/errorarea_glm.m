function [linehandles, patchhandles] = errorarea_glm(x,y,l,u)
...
for ln = 1:nlns
    ...
    ywrap = [u(:,ln)',fliplr(l(:,ln)')]; %% MKL
% 	ywrap = [fliplr(y(:,ln)+u(:,ln))', (y(:,ln)-l(:,ln))']; %% MKL
%     ywrap = [(y(:,ln)+abs(u(:,ln)))', flipud(y(:,ln)-abs(l(:,ln)))'];
