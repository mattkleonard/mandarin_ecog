function [out_els,out_els_ind]=electrodes2surf_array(subject,index,checkdistance,elecselection,elecfname,gsfname,mrfname)
% function
...
out_els=p_zoom_array(els,gs,index);
% data.gs=[];
% data.gsStruct=[];
% data.gsName=[];
% clear gs;
...
% put back in image:
if ~exist(mrfname)
    [datawithnewelec,out_els,out_els_ind]=position2reslicedImage2(out_els);
else
    [datawithnewelec,out_els,out_els_ind]=position2reslicedImage2(out_els,mrfname);
end
